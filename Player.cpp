...
void update_player(Player* player, const GameInput &game_input, const Camera3D &camera, double dt)
{
    ...
    if(player_move_length2 > 0.00001)
    {
        ...
        {
            const float player_turn_speed = 720.f;
    ...
    player_accel -= friction_factor * (vec3){player->vel.x, 0, player->vel.z};
    ...
    const float player_gravity = -2 * player_jump_height * (player_jump_time_to_peak * player_jump_time_to_peak);
    const float jump_vel = 2 * player_jump_height * player_jump_time_to_peak;
    ...
    else //Player is not on ground 
    {
        ...
        if(player->is_jumping){
            ...
            if(!game_input.is_down[JUMP] && (player->vel.y > 0)) 
                gravity_scale = 4.0f;
        }
