...
void Camera3D::update_debug(double dt){
    ...
    if(!cam_mouse_controls){
        if(g_move_input[TURN_CAM_LEFT]) {
            yaw += g_move_input[TURN_CAM_LEFT]*turn_speed*dt;			
        }
        if(g_move_input[TURN_CAM_RIGHT]) {
            yaw -= g_move_input[TURN_CAM_RIGHT]*turn_speed*dt;			
        }
        if(g_move_input[TILT_CAM_UP]) {
            pitch += g_move_input[TILT_CAM_UP]*turn_speed*dt;			
        }
        if(g_move_input[TILT_CAM_DOWN]) {
            pitch -= g_move_input[TILT_CAM_DOWN]*turn_speed*dt;			
        }
    }
    else {
