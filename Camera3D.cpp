...
void init_camera(Camera3D* cam, vec3 cam_pos, vec3 target_pos){
    ...
    cam->yaw   = RAD2DEG( acos(cam->V.m[0]));
    cam->pitch = RAD2DEG(-acos(cam->V.m[5])); //no idea why you negate this and not yaw, it just works
	cam->move_speed = 10;
	cam->turn_speed = 100;
}
...
void update_camera(Camera3D* cam, CameraMode cam_mode, vec3 player_pos, double dt)
{
    ...
    cam->rgt = R*vec4(1,0,0,0);
    cam->up  = R*vec4(0,1,0,0);
    cam->fwd = R*vec4(0,0,-1,0);
